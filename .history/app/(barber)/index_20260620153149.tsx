...
import { View, Text, FlatList, Modal, TouchableOpacity, Pressable } from 'react-native'
...
function AppointmentCard({
  ...
  return (
    <View className='bg-white rounded-xl border border-slate-200 mb-3 overflow-hidden flex-row'>
      {/* Color stripe */}
      <View
        ...
        >
          ...
          >
            ...
            >
              ...
              {Object.entries(statusConfig).filter(([status]) => status !== item.status).map(([status, config]) => (
                <TouchableOpacity
                  key={status}
                  className={`py-4 px-3 rounded-lg mb-2 ${config.bg}`}
                  onPress={() => {
                    onChangeStatus(item.id, status as appointment_status)
                    setShowStatusModal(false)
                  }}
                >
                  <Text className={`text-base ${config.text}`}>
                    {config.label}
                  </Text>
                </TouchableOpacity>
              ))}
...
export default function BarberDashboard() {
  ...
  return (
    <View className='flex-1 bg-slate-50'>
      ...
      {/* View mode selector */}
      <View className='flex-row mx-4 mb-3 bg-slate-100 rounded-xl p-1'>
