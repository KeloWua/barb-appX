      ...
      <View className='flex-1 p-4'>
        ...
        {item.status === 'confirmed' && (
          <View className='flex-row gap-2'>
            <TouchableOpacity
              className={`flex-1 bg-blue-600 rounded-lg py-2 items-center ${isChangingStatus ? 'opacity-50' : ''}`}
            ...
            <TouchableOpacity
              className={`flex-1 bg-slate-100 rounded-lg py-2 items-center ${isChangingStatus ? 'opacity-50' : ''}`}
              onPress={() => onNoShow(item.id)}
              disabled={isChangingStatus}
            >
              <Text className='text-slate-600 font-semibold text-sm'>No show</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {item.status === 'confirmed' && (
          <TouchableOpacity
            className={` bg-green-600 rounded-lg py-2 items-center ${isChangingStatus ? 'opacity-50' : ''}`}
            onPress={() => onComplete(item.id)}
            disabled={isChangingStatus}
          >
            <Text className='text-white font-semibold text-sm'>Marcar completada</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <AppointmentCard
              ...
              onConfirm={(id) => changeStatus({ id, status: 'confirmed' })}
              onComplete={(id) => changeStatus({ id, status: 'completed' })}
              onNoShow={(id) => changeStatus({ id, status: 'no_show' })}
            />
          )}
        />
      )}
