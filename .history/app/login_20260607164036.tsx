    ...
    return (
        <View className="flex-1 justify-center px-6 bg-white">
            ...
            <Controller control={control} name="password" render={({ field: {
                ...
                <View className="mb-6">
                    <TextInput className="border border-slate-300 rounded-lg p-4 bg-slate-50"
                    placeholder="Password"
                    secureTextEntry
                    value={value}
                    onChangeText={onChange}
                    />
                    {errors.password && <Text className="text-red-500 mt-1">
                        </Text>}
                </View>
                )}/>
                <TouchableOpacity
                className={`bg-blue-600 rounded-lg p-4 items-center ${isLoading ? 'opacity-50' : ''}`}
                onPress={handleSubmit(onsu)}
                >
