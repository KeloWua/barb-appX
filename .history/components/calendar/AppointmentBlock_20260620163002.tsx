...
export function AppointmentBlock({ appointment, onPress }: Props) {
    ...
    return (
        ...
        >
            ...
            <Text className='text-[10px] text-blue-700 truncate mt-0.5'>
                {appointment.services?.name_es || 'Servicio'}
            </Text>
            <Text className='text-[10px] text-blue-500 font-semibold mt-auto'>
                {new Date(appointment.start_time).toLocaleTimeString([], { hour:})}
            </Text>
        </TouchableOpacity>
    )
}